...
## Notes
* There are`_PLD` methods exist under each ports in my DSDT, refer to the _PLD in ACPI Spec for more info.
* This idea of `_STA`ing, and re-assigning `_ADR` was based on SSDT-USB-Reset generated by USBMap. Information regarding `_UPC` method are from 5T33Z0's ACPI USB Mapping guide.
